...
* Python source files:
  * The Python package source files are located in the `src/test_gha_python_package` directory.
  * `tests/test_test_gha_python_package.py` contains the unit tests for the package.
  * `tests/conftest.py` contains testing setup and configuration for `pytest`
  * The `notebooks` directory contains an example Jupyter notebook on how to use `test_gha_python_package`.
